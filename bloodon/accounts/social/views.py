...
from bloodon.accounts.social import helpers
...
class ConnectionsView(FormView):
    ...
    def form_valid(self, form):
        ...
        AccountHelper().add_message(self.request,
                                          messages.INFO,
...
def social_login(request, provider):
    if provider == "facebook":
        from .providers.facebook.views import oauth2_login

        return oauth2_login(request)
    elif provider == "google":
        from .providers.google.views import oauth2_login
...
def social_callback(request, provider):
    if provider == "facebook":
        from .providers.facebook.views import oauth2_callback

        return oauth2_callback(request)
    elif provider == "google":
        from .providers.google.views import oauth2_callback
